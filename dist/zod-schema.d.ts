...
export declare const serializedDateSchemaForSerialize: z.ZodEffects<z.ZodUnion<[z.ZodString, z.ZodNumber, z.ZodDate, z.ZodType<Dayjs, z.ZodTypeDef, Dayjs>]>, `${number}-${number}${number}-${number}${number}T${number}${number}:${number}${number}:${number}${number}.${number}${number}${number}Z`, string | number | Dayjs | Date>;
